...
constexpr data_T minval() {
    if constexpr (std::numeric_limits<data_T>::is_specialized){
        return std::numeric_limits<data_T>::lowest();
    }else{
        if(!data_T::sign) return data_T(0);
        else return data_T(-(1LL << (data_T::i_width - 1)));
    }
}
...
struct CausalState {
    static constexpr unsigned BRAM_SIZE = CONFIG_T::n_ctx *
        CONFIG_T::n_inplace * CONFIG_T::n_contract * CONFIG_T::n_free1;
...
template<typename data_T, typename CONFIG_T>
device_global<typename CausalState<data_T,CONFIG_T>::State> causal_state;
...
template<class data0_pipe, class data1_pipe, class res_pipe, typename CONFIG_T>
void causal_einsum() { 
        ...
        for(unsigned i = 0; i < I; i++){
            ...
            if constexpr (!CONFIG_T::contract_dim){// CONTRACT ALONG THE C DIMENSION
                ...
                for(unsigned l0 = 0; l0 < L0; l0++){
                    ...
                    if(l0 == 0) read_causal_pipe<data1_T, data1_pipe, CONFIG_T>(i, write_ptrs, ctx_cts, causal_buff);
                    ...
                    for(unsigned ctx = 0; ctx < CTX; ctx++){
                        ...
                        if(ctx < ctx_cts[i]){
                            ...
                            #pragma unroll
                            for(unsigned l1 = 0; l1 < L1; l1++){
                                res_buffer[ctx_buff_offset + l1] = minval<res_T>();//res_T(0);//no need to pad with -INF since V mult has 0's in place 
                            }
                        }
                    }
                ...
                for(unsigned l1 = 0; l1 < L1; l1++){
                    ...
                    for(unsigned ctx = 0; ctx < CTX; ctx++){
                        if(ctx < ctx_cts[i]) tmp += data_vect_buffer[ctx] * causal_buff[L1 * I * ((offset_ctx + ctx) % CTX) + L1 * i + l1];
                    }
